...
export const PLANNING_EMPTY_COPY = {
  ...
  resolutionUnavailable:
    'We could not confirm your municipality yet. Confirm where you live to see local municipal information.',
  municipalitySnapshotComingSoon: 'Municipal information coming soon',
  municipalitySnapshotComingSoonBody:
    'We have identified your municipality. Verified planning and budget information has not yet been published on Serve SA.',
} as const

/** Deterministic major-project selection for citizen Municipality Snapshot */
export const MUNICIPALITY_SNAPSHOT_PRIORITY_LIMIT = 6
export const MUNICIPALITY_SNAPSHOT_PROJECT_LIMIT = 8

function scopeRank(scope: string | undefined): number {
  if (scope === 'municipality_wide') return 0
  if (scope === 'regional') return 1
  if (scope === 'ward_specific') return 3
  return 2
}
 ...
 */
export function selectMajorMunicipalProjects<
  T extends {
    scope?: string
    status?: string
    sortOrder?: number
  },
>(projects: T[], limit = MUNICIPALITY_SNAPSHOT_PROJECT_LIMIT): T[] {
  return [...projects]
