...
1. **Ops surface** lives at `/ops` (Dashboard, Cases, Case detail, Team, Settings) — not an admin analytics portal.
2. **All privileged mutations** (status, assign, notes, public updates, departments, claims) are Cloud Functions / Admin SDK only. Clients cannot write `cases`.
...
5. **Internal notes** and **public updates** are separate subcollections.
6. Custom claims (`roles`, `municipalityCode`) gate Firestore reads and callable auth.
