...
test.describe('Official UAT @pilot', () => {
  ...
  test('ops page loads (gate or queue UI) @pilot', async ({ page }) => {
    ...
    // Either access gate / sign-in redirect, or queue chrome when authenticated
  ...
  test('work queue UI structure when official token present', async ({ page }) => {
    ...
    await expect(page.getByText(/needs acknowledgement|duplicate review|all actions/i).first()).toBeVisible()
  })
  ...
  test('unauthenticated ops does not render municipality case rows @pilot', async ({
    ...
    // Should not show a populated queue of case references without auth
